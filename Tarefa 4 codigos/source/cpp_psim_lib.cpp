...
extern "C" {
    ...
    DLLEXPORT void SimulationBegin(
        ...
        char msgReceived[8];
        writeSerial("eightchr"); // Escreve a mensagem na porta
        Sleep(50); // Espera pela resposta do DSP
        readSerial(msgReceived, 8); // Lê a resposta do DSP e salva numa variável
        closeSerial();

        openSerial("\\\\.\\COM1"); // Abre porta virtual
        writeSerial("\nReceived:");
        writeSerial(msgReceived); //Escreve Mensagem recebida do DSP
